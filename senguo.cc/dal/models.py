...
class Fruit(MapBase, _CommonApi):
	...
	add_time = Column(DateTime,nullable=False, default=func.now()) #5.27
...
class Article(MapBase, _CommonApi):
	...
	title = Column(String(100),nullable=False)
	article = Column(Text)
	classify = Column(TINYINT,default = 0) #0:官方公告 1:产品更新 2:运营干货 3:水果百科 4:使用教程 5:水果供求
	great_num = Column(Integer,default = 0) #点赞数
	comment_num = Column(Integer,default = 0)#评论数
	scan_num = Column(Integer,default = 0) #0:浏览数
	if_scan = Column(TINYINT,default = 0) #0:是否浏览
	status = Column(TINYINT,default = 1) # -1:草稿 0:删除 1:正常 2:定时发布
