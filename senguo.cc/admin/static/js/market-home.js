$(document).ready(function () {
...
}).on('click', '.check-lg-img', function (e) {
    e.stopPropagation();
    //查看大图
    var $this = $(this);
    ...
    if (if_favour == 'true') {
        large_box.find('.click-great').addClass('clicked').removeClass('able_click')
    }
    else {
        large_box.find('.click-great').removeClass('clicked').addClass('able_click');
    }
...
}).on('click', '.number-minus', function (e) {
    e.stopPropagation();
    //商品数量操作
    var $this = $(this);
    pulse($this);
    goodsNum($this, 1);
}).on('click', '.number-plus', function (e) {
    e.stopPropagation();
    var $this = $(this);
    var parent = $this.parents('.goods-list-item');
    ...
    if (num == 0) {
        ...
        if (result > 0) {
            parent.attr({'data-storage': result});
        }
...
function goodsNum(target, action) {
    ...
    if (type_list.hasClass('fruit-list')) {
        window.dataObj.fruits[id] = num;
        fruits_num();
    }
    if (type_list.hasClass('menu-list')) {
        window.dataObj.mgoods[id] = num;
        mgoods_num();
    }
}
...
function fruits_num() {
    for (var key in window.dataObj.fruits) {
        if (window.dataObj.fruits[key] == 0) {
            delete window.dataObj.fruits[key];
        }
    }
}
...
function mgoods_num() {
    for (var key in window.dataObj.mgoods) {
        if (window.dataObj.mgoods[key] == 0) {
            delete window.dataObj.mgoods[key];
        }
    }
}
...
function Swipe(b, g) {
    ...
    var z = {addEventListener: !!window.addEventListener, touch: ("ontouchstart" in window) || window.DocumentTouch && document instanceof DocumentTouch, transitions: (function (A) {
    ...
    var v, h, o;
    g = g || {};
    var f = parseInt(g.startSlide, 10) || 0;
    var s = g.speed || 300;
    g.continuous = g.continuous ? g.continuous : true;
    function m() {
        v = i.children;
        h = new Array(v.length);
        o = b.getBoundingClientRect().width || b.offsetWidth;
        i.style.width = (v.length * o) + "px";
        var B = v.length;
        while (B--) {
            var A = v[B];
            A.style.width = o + "px";
    ...
    function a(D, A) {
        ...
        if (z.transitions) {
            var B = Math.abs(f - D) - 1;
            var C = Math.abs(f - D) / (f - D);
            while (B--) {
                p((D > f ? D : f) - B - 1, o * C, 0)
            }
            p(f, o * C, A || s);
    ...
    function e(E, D, A) {
        ...
        var B = setInterval(function () {
            ...
            if (F > A) {
                ...
                g.transitionEnd && g.transitionEnd.call(event, f, v[f]);
                clearInterval(B);
                return
            }
            i.style.left = (((D - E) * (Math.floor((F / A) * 100) / 100)) + E) + "px"
        }, 4)
    }
    ...
    var j = {handleEvent: function (A) {
        ...
        var B = A.touches[0];
        d = {x: B.pageX, y: B.pageY, time: +new Date};
        x = undefined;
        w = {};
        i.addEventListener("touchmove", this, false);
        i.addEventListener("touchend", this, false)
    }, move: function (A) {
        ...
        if (!x) {
            A.preventDefault();
            q();
            w.x = w.x / ((!f && w.x > 0 || f == v.length - 1 && w.x < 0) ? (Math.abs(w.x) / o + 1) : 1);
            k(f - 1, w.x + h[f - 1], 0);
            k(f, w.x + h[f], 0);
            k(f + 1, w.x + h[f + 1], 0)
        }
        ...
        var A = Number(E) < 250 && Math.abs(w.x) > 20 || Math.abs(w.x) > o / 2;
        var B = !f && w.x > 0 || f == v.length - 1 && w.x < 0;
        var D = w.x < 0;
        if (!x) {
            if (A && !B) {
                if (D) {
        ...
        i.removeEventListener("touchmove", j, false);
        i.removeEventListener("touchend", j, false)
    }, transitionEnd: function (A) {
        if (parseInt(A.target.getAttribute("data-index"), 10) == f) {
            if (y) {
                u()
            }
            g.transitionEnd && g.transitionEnd.call(A, f, v[f])
        }
    }};
    m();
    if (y) {
    ...
    if (z.addEventListener) {
        if (z.touch) {
            i.addEventListener("touchstart", j, false)
        }
        if (z.transitions) {
            i.addEventListener("webkitTransitionEnd", j, false);
            i.addEventListener("msTransitionEnd", j, false);
            i.addEventListener("oTransitionEnd", j, false);
            i.addEventListener("otransitionend", j, false);
            i.addEventListener("transitionend", j, false)
        }
    ...
    return{setup: function () {
        ...
        if (z.addEventListener) {
            i.removeEventListener("touchstart", j, false);
            i.removeEventListener("webkitTransitionEnd", j, false);
            i.removeEventListener("msTransitionEnd", j, false);
            i.removeEventListener("oTransitionEnd", j, false);
            i.removeEventListener("otransitionend", j, false);
            i.removeEventListener("transitionend", j, false);
            window.removeEventListener("resize", j, false)
        } else {
