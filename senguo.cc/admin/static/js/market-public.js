$(document).ready(function(){
 ...
 //    });
    ...
    //      timestamp:window.dataObj.timestamp_val, // 必填，生成签名的时间戳
    //      nonceStr:window.dataObj.noncestr_val, // 必填，生成签名的随机串
    //      signature:window.dataObj.signature_val,// 必填，签名，见附录1
    //      jsApiList: ['onMenuShareTimeline','onMenuShareAppMessage'] // 必填，需要使用的JS接口列表，所有JS接口列表见附录2
    //  });
    //  wx.ready(function(){
    //      wx.onMenuShareTimeline({
    //      title: '', // 分享标题
    //      link:'/shop/'+shop_code, // 分享链接
    //      imgUrl: '', // 分享图标
    //      success: function () {
    //      // 用户确认分享后执行的回调函数
    //      },
...
function wexin(){
    ...
    $.postJson(url,args,function(res){
        if(res.success){
            window.dataObj.noncestr_val=res.noncestr;
            window.dataObj.timestamp_val=res.timestamp;
            window.dataObj.signature_val=res.signature;
        }
