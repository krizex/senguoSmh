...
$(document).ready(function(){
    ...
    if(mWidth>800){
        ...
        $("#more-get").css("left",((mWidth-width)/2+width-96)+"px");
    }
    ...
    //初始化购物车数量
    if(cookie.getCookie("cart_count")!=''){
        $("#cart-now-num").html(cookie.getCookie("cart_count")).removeClass("move-cart");
    }
    ...
    $(".add-btn").on("click",function(){
        ...
        var relate=parseFloat($this.parents(".progress-num").attr('data-relate'));
        var unit_num=parseFloat($this.parents(".progress-num").attr('data-unit'));
        var storage=parseFloat($this.parents(".progress-num").attr("data-storage"));
        var limit_num=parseInt($this.parents(".wrap-goods-detail").attr('data-limit'));
        var change_num=relate*unit_num*1;
        var buy_today=$this.parents('.progress-num').attr('data-buy');
        var allow_num=parseInt($this.parents('.progress-num').attr('data-allow') || "");
