$(document).ready(function(){
    var item_url='/static/items/admin/order-item.html?v=20160608';
    //订单数据
...
function getOrder(url){
    $.getItem(url,function(data){
            $list_item=data;
            //商品列表item
    	    getGoodsItem('/static/items/admin/order-goods-item.html?v=20160608');
    	    //员工列表item
    	    getStaffItem('/static/items/admin/order-staff-item.html?v=20160608');
            orderItem(orders);
        }
    );
}
...
function orderPrint(target,action){
    ...
    if(action =='print'){
        ...
        function getData(target){
            ...
            $.getItem('/static/items/admin/order-print-page.html?v=20160608',function(data){
                var $item=$(data);
