$(document).ready(function(){
	...
	var $this=$(this);
              $('.getCode').attr({'disabled':true}).addClass('bg-grey');
	Vrify($this);
            ...
            checkCode($this);
             ...
             var $this=$(this);
             $('.reset').attr({'disabled':true}).addClass('bg-grey');
            pwdRest($this);
...
function Vrify(target){
    ...
    $.postJson(url,args,
	  ...
	  $('.progress2').addClass('active').siblings('.progress').removeClass('active');
                $('.getCode').removeAttr('disabled').removeClass('bg-grey');
            }
            else {
...
function pwdRest(target){
    ...
    if(!password) {return noticeBox('密码不能为空!',target);}
    if(re_password!=password) {return noticeBox('两次输入的密码不一致!',target);}
    if(password.length<6 || !regPass.test(password)) {return noticeBox('请输入六位以上字母和数字的组合!',target);}
    ...
    $.postJson(url,args,
        function(res){
            if(res.success)
            {
                ...
                noticeBox(res.error_text);
                $('.reset').removeAttr('disabled').removeClass('bg-grey');
            }
        },
        function(){
            noticeBox('网络好像不给力呢~ ( >O< ) ~');
